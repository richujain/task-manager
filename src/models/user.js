...
const userSchema = new mongoose.Schema({
    ...
    email: {
        type: String,
        unique: true, //Multiple accounts won't have same email
        required: true,
        trim: true,
        lowercase: true,
        validate(value){
...
userSchema.statics.findByCredentials = async (email, password) => {
    ...
    if(!user){
        throw new Error('Unable to login with that Email ID')
    }
    const isMatch = await bcrypt.compare(password, user.password)
    
    if(!isMatch){
        throw new Error('Invalid Password.') //It's better not to be specific in error. 
    }
...
userSchema.pre('save', async function (next) {
    const user = this // we use the regular function to use "this"
